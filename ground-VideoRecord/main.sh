...
echo "Checking rtsp-simple-server status..." | ts '[%Y-%m-%d %H:%M:%S]' >> $INSTALLPATH/data/log/rtsp_run.log
if [ -z "$RTSP_PID" ]
then
      echo "rtsp-simple-server not running, thus starting it now." | ts '[%Y-%m-%d %H:%M:%S]' >> $INSTALLPATH/data/log/rtsp_run.log
      $INSTALLPATH/rtsp-simple-server >> $INSTALLPATH/data/log/rtsp_run.log 2>&1 &
else
      echo "rtsp-simple-server is allready running with PID: $RTSP_PID" | ts '[%Y-%m-%d %H:%M:%S]' >> $INSTALLPATH/data/log/rtsp_run.log
fi
